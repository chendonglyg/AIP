Public Class MAOYI
    ...
    Private Sub showall()
        ...
        If TreeView1.SelectedNode.Text = "--超时车辆" Then
            objdataadapter.SelectCommand.CommandText = " select * from TRUCK_QUEUE where Overtimeflag=1 order by 进厂时间"
            If objconnection.State = ConnectionState.Closed Then
                objconnection.Open()
            End If
            objdataset = New DataSet
            ...
            kedengji.DataSource = objdataview
            kedengji.ContextMenuStrip = BackToParking
            Box2.Text = "超时车辆"
            C1.Items(1).Enabled = False
    ...
    Private Sub TreeView1_AfterSelect(sender As Object, e As TreeViewEventArgs) Handles TreeView1.AfterSelect
        ...
        If TreeView1.SelectedNode.Text = "已完成" Then
            ...
            objdataview = New DataView(objdataset.Tables("yiwancheng"))
            kedengji.ContextMenuStrip = DoneToParking
            kedengji.DataSource = objdataview
            box2.Text = "已完成数据"
    ...
    Private Sub kedengji_CellContentClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        pici.Text = Trim(kedengji.Rows(e.RowIndex).Cells(22).Value)
    ...
    Private Sub kedengji_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs)
    ...
    Private Sub 退回操作ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles C1_1.Click
        Try
    ...
    Private Sub chaxun_KeyDown(sender As Object, e As KeyEventArgs) Handles chaxun.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            '-----------------------------------------------------------------------------------------------------
            If TreeView1.SelectedNode.Text = "--超时车辆" Then
                objdataadapter.SelectCommand.CommandText = " select * from TRUCK_QUEUE where Overtimeflag=1 and 车号 like '%" & Trim(chaxun.Text) & "%'"
                If objconnection.State = ConnectionState.Closed Then
                    objconnection.Open()
                End If
                objdataset = New DataSet
    ...
    Private Sub force_Click(sender As Object, e As EventArgs) Handles C1_2.Click
        'VPN没有+无论任何情况都可以登记
        Dim objcommandpassword As New SqlClient.SqlCommand
        objcommandpassword.Connection = objconnection
        objcommandpassword.CommandText = "update TRUCK_QUEUE set 可登记=0,已登记=1,登记时间=@登记时间,备注=@备注" & " where 车号='" & Trim(chehao.Text) & "'" 'and 导入批次='" & Trim(pici.Text) & "'"
    ...
    Private Sub 配卡通行ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles C1_3.Click
        港务局测试一卡通专用.Show()
    ...
    Private Sub ToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItem1.Click
        Try
            ...
            If Len(chehao.Text) > 0 Then
                ...
                Dim objcommandpassword As New SqlClient.SqlCommand
                objcommandpassword.Connection = objconnection
                objcommandpassword.CommandText = "update TRUCK_QUEUE set 可登记=1,overtimeflag=0,已退回=0,备注=@备注" & " where id=" & Val(idid.Text)
                ...
                objcommandpassword.ExecuteNonQuery()
                objconnection.Close()
                MessageBox.Show("当前车辆已被退回至停车场重新登记!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information)

                '-------------------------------------
                sendsomething.sendOneMessage(Trim(dianhuahaoma.Text), "<<车辆退回信息>>" & Trim(kehumingcheng.Text) & "车号:" & Trim(chehao.Text) & "因超时等原因被退回至停车场重新登记!", "益海贸易部", "fpiyx,LUGXH^7673bxgs.68423#$%##dwt")
    ...
    Private Sub chaxun_TextChanged(sender As Object, e As EventArgs) Handles chaxun.TextChanged
    ...
    Private Sub 强制放行ToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles C1_4.Click

        Dim objcommandpassword As New SqlClient.SqlCommand
        objcommandpassword.Connection = objconnection
        objcommandpassword.CommandText = "Update TRUCK_QUEUE set 可登记=0,已登记=0,可进厂=1,进厂时间=@进厂时间 where 车号='" & Trim(chehao.Text) & "'"  '--------------------只要放管制车辆，那么该车号所有其他非管制货物也将全部放行
        ...
        objcommandpassword.ExecuteNonQuery()
        objconnection.Close()
        MessageBox.Show("已强制放行!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub ToolStripMenuItem2_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItem2.Click
        Try
            If Len(chehao.Text) > 0 Then
                ...
                objdataadapter.SelectCommand.CommandText = "UPDATETRUCKDoneToParking"
                ...
                MessageBox.Show("完成后退操作!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                showall()
                sendsomething.sendOneMessage(Trim(dianhuahaoma.Text), "<<车辆登记提醒>>" & Trim(kehumingcheng.Text) & "," & Trim(pinming.Text) & ",提单量:" & Trim(shuliang.Text) & "吨," & Trim(chehao.Text) & "可进行登记(退回记录)", "益海贸易部", "fpiyx,LUGXH^7673bxgs.68423#$%##dwt")
    ...
    Private Sub kedengji_CellContentClick_1(sender As Object, e As DataGridViewCellEventArgs) Handles kedengji.CellContentClick
        On Error Resume Next
        idid.Text = Trim(kedengji.Rows(e.RowIndex).Cells(0).Value)
        kehumingcheng.Text = Trim(kedengji.Rows(e.RowIndex).Cells(1).Value)
