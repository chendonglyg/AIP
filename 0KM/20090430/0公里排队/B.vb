Public Class PARKING
    ...
    Private Sub kedengji_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles kedengji.CellContentClick
        ...
        If TreeView1.SelectedNode.Text = "可登记" Then
            ...
            ' stre = "success"             '打开后任何车辆都可强行登记

            '---------------------------------------
            If chehao.Text = "测试123" Then
                stre = "success"
            End If
    ...
    Private Sub dengji_Click(sender As Object, e As EventArgs) Handles dengji.Click
        ...
        Try
            '----------------------------------------------------------------------------------这块代码验证在点击正常登记时候，是否预约满足条件，避免预约满足时依然进入正常登记
            Dim timenow As Date
            Dim isReservedflag As Boolean

            timenow = sendsomething.getDateTime()

            If Len(Reservedtime1.Text) > 0 And Len(Reservedtime2.Text) > 0 Then

                If isReserved.CheckState = CheckState.Checked And (timenow > Trim(Reservedtime1.Text) And timenow < Trim(Reservedtime2.Text)) Then
                    isReservedflag = False
                Else
                    isReservedflag = True
                End If
            ...
            If Len(IDID.Text) > 0 And isReservedflag Then


                Dim objcommandpassword As New SqlClient.SqlCommand
                objcommandpassword.Connection = objconnection
                objcommandpassword.CommandText = "Update TRUCK_QUEUE set 可登记=0,已登记=1,登记时间=@登记时间,备注=@备注,isReserved=0,Overtimeflag=0,已退回=0" & " where id=" & Val(IDID.Text)
            ...
            Else
                MessageBox.Show("没有选择被记录或该车辆满足预约条件", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub ToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItem1.Click
        B1.Show()
        B1.chehao.Text = Trim(chehao.Text)
        B1.dianhuahaoma.Text = Trim(dianhuahaoma.Text)
