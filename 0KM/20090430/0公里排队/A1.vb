Public Class A1
    ...
    Dim objdataadapter, objdataadapterissave As New SqlClient.SqlDataAdapter
    Dim sendsomething As New sendmessage.CTEnterprisesSoapClient
    Dim objdataadapterissavereader As SqlClient.SqlDataReader
        ...
        Else
            ...
            For c = a To b
                ...
                myRow("客户名称") = Trim(xlssheet.cells(c, 2).value())
                    myRow("司机姓名") = Trim(xlssheet.cells(c, 3).value())
                ...
                myRow("车号") = Trim(xlssheet.cells(c, 7).value())
                    ...
                    If xlssheet.cells(c, 8).value() = "连云港" Or xlssheet.cells(c, 8).value() = "供方工厂" Then
                        myRow("自派车") = 0  '自提
                    Else
                        myRow("自派车") = 1   '自派
                    End If
                    ...
                    myRow("司机二代证") = Trim(xlssheet.cells(c, 10).value())



                    If Val(xlssheet.cells(c, 11).value()) = 1 Then
                        myRow("isReserved") = Val(xlssheet.cells(c, 11).value())
                        myRow("Reserveredtime1") = Trim(xlssheet.cells(c, 12).value())


                        Dim Reserveredtime2Value As Date

                        Reserveredtime2Value = Trim(xlssheet.cells(c, 12).value())
                    ...
                    myRow("备注") = Trim(xlssheet.cells(c, 13).value())

                    myRow("件数") = Trim(xlssheet.cells(c, 14).value())
                    myRow("导入批次") = pici
                    ...
                    '------------------------------------------------------------------------------------------------------------要判断是否有重复的提单号&车号
                    objdataadapterissave = New SqlClient.SqlDataAdapter
                    objdataadapterissave.SelectCommand = New SqlClient.SqlCommand
                    objdataadapterissave.SelectCommand.Connection = objconnection
                    objdataadapterissave.SelectCommand.CommandType = CommandType.StoredProcedure

                    objdataadapterissave.SelectCommand.CommandText = "UPDATETRUCKNOISSAVE"



                    objdataadapterissave.SelectCommand.Parameters.AddWithValue("@chehao", Trim(xlssheet.cells(c, 7).value()))
                    objdataadapterissave.SelectCommand.Parameters.AddWithValue("@tidanhao", Trim(xlssheet.cells(c, 4).value()))




                    objdataadapterissave.SelectCommand.Parameters.AddWithValue("@RETURN_VALUE", "").Direction = ParameterDirection.ReturnValue
                    ...
                    objdataadapterissave.SelectCommand.ExecuteNonQuery()

                    If (objdataadapterissave.SelectCommand.Parameters("@RETURN_VALUE").Value) = 1 Then
                        MessageBox.Show("发现导入的数据有重复，系统将忽略这些重复，其他数据将正常导入!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        GoTo issave
                    End If
                    '------------------------------------------------------------------------------------------------------------如果有重复数据则跳往issave节点
...
NOFILE:
    ...
    Private Sub ToolStripTextBox1_KeyDown(sender As Object, e As KeyEventArgs) Handles Txt_PICI.KeyDown
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        objdataadapterissave.SelectCommand.CommandText = "UPDATETRUCKNOISSAVE"
            ...
            objdataadapterissave.SelectCommand.Parameters.AddWithValue("@chehao", Trim("苏G511328"))
            objdataadapterissave.SelectCommand.Parameters.AddWithValue("@tidanhao", Trim("8135795431"))
            ...
            MessageBox.Show(objdataadapterissave.SelectCommand.Parameters("@RETURN_VALUE").Value)
